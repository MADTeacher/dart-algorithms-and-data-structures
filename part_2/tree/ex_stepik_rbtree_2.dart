...
class RBTree<T extends IKey> {
  ...
  // Задача 2: Подсчет черных узлов
  int get totalBlackNodes {
    return _countBlackNodes(_root);
  }

  int _countBlackNodes(_Node<T>? node) {
    if (node == null) return 0;
    int count = _getNodeColor(node) == _Color.black ? 1 : 0;
    count += _countBlackNodes(node.left);
    count += _countBlackNodes(node.right);
    return count;
  }
...
void main() {
  ...
  print('Вставляем элементы: 10, 5, 20');
  tree.insert(Worker('A', 10));
  tree.insert(Worker('B', 5));
  tree.insert(Worker('C', 20));
  tree.printTree();

  // При вставке 3-х элементов (10, 5, 20) — корень черный, дети красные -> ответ 1
  ...
  print('\n--- Дополнительный тест ---');
  final tree2 = RBTree<Worker>();
  final keys = [10, 5, 20, 3, 7, 15, 25];
  print('Вставляем элементы: ${keys.join(", ")}');
  for (final key in keys) {
    tree2.insert(Worker('Worker', key));
  }
  tree2.printTree();
