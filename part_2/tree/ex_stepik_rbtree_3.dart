...
class RBTree<T extends IKey> {
  ...
  // Задача 3: Функциональная свертка (Fold/Reduce)
  R fold<R>(R initialValue, R Function(R previous, T element) combine) {
    return _foldSymmetric(_root, initialValue, combine);
  }
  ...
  R _foldSymmetric<R>(
    ...
    accumulator = _foldSymmetric(node.left, accumulator, combine);
    accumulator = combine(accumulator, node.data);
    accumulator = _foldSymmetric(node.right, accumulator, combine);
    return accumulator;
  }
...
void main() {
  ...
  print('Вставляем Worker-ов с id: 1, 2, 3');
  tree.insert(Worker('Alice', 1));
  tree.insert(Worker('Bob', 2));
  tree.insert(Worker('Charlie', 3));
  tree.printTree();

  // Задача: посчитать сумму всех ID
