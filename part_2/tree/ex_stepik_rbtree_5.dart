...
class RBTree<T extends IKey> {
  ...
  // Задача 5: Проверка суммы пути (Path Sum)
  bool hasPathSum(int targetSum) {
    if (_root == null) return false;
    return _hasPathSumRecursive(_root, 0, targetSum);
  }

  bool _hasPathSumRecursive(_Node<T>? node, int currentSum, int targetSum) {
    if (node == null) return false;

    final newSum = currentSum + node.key;

    // Проверяем, является ли узел листом (оба потомка null)
    if (node.left == null && node.right == null) {
      return newSum == targetSum;
    }

    // Рекурсивно проверяем левое и правое поддеревья
    return _hasPathSumRecursive(node.left, newSum, targetSum) ||
        _hasPathSumRecursive(node.right, newSum, targetSum);
  }
