...
({int index, T? value}) ternarySearch<T>(
  ...
  // вызываем рекурсивную функцию для поиска
  return _ternarySearch_impl(list, compare, 0, list.length - 1);
}

// Рекурсивная функция тернарного поиска
// list - отсортированный список элементов
// compare - функция сравнения элемента с искомым значением
...
({int index, T? value}) _ternarySearch_impl<T>(
  ...
  // Проверяем, что левая граница не превышает правую
  if (right > left) {
    // Если осталось только два элемента для проверки
    if (right - left == 1) {
      ...
      // Элемент не найден в оставшихся двух позициях
      return (index: -1, value: null);
    }

    // Вычисляем размер шага для деления массива на три части
    // Если диапазон меньше 3 элементов, используем шаг 1
    var step = (right - left) < 3 ? 1 : ((right - left) ~/ 3);

    // Вычисляем две точки деления массива на три части
    var middle1 = left + step; // Первая треть
    var middle2 = middle1 + step; // Вторая треть

    // Проверяем, найден ли искомый элемент в первой точке деления
    if (compare(list[middle1]) == 0) {
      return (index: middle1, value: list[middle1]);
    }

    // Проверяем, найден ли искомый элемент во второй точке деления
    if (compare(list[middle2]) == 0) {
      return (index: middle2, value: list[middle2]);
    }

    // Определяем, в какой трети массива продолжить поиск
    if (compare(list[middle1]) > 0) {
      // Искомый элемент меньше middle1, ищем в левой трети
      return _ternarySearch_impl(list, compare, left, middle1);
    } else if (compare(list[middle1]) > 0 && compare(list[middle2]) < 0) {
      // Искомый элемент между middle1 и middle2, ищем в средней трети
      return _ternarySearch_impl(list, compare, middle2, middle2);
    } else {
      // Искомый элемент больше middle2, ищем в правой трети
      return _ternarySearch_impl(list, compare, middle2, right);
    }
  }
  // Если left >= right, элемент не найден
  return (index: -1, value: null);
}
